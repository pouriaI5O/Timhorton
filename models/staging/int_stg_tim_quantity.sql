with talbot_255 as (SELECT  *,
1642 as branch_id,'TH-ON-Leamington-1642-255-Talbot'as branch   from {{ source('public','talbot_255') }} where drive_thru_time is not null order by date ),
 maidstone_1903 as (SELECT  *,
1903 as branch_id,'TH-ON-Essex-1903-9-Maidstone' as branch   from {{ source('public','maidstone_1903') }} where drive_thru_time is not null order by date ),
erie185 as (SELECT  *,
562 as branch_id,'TH-ON-Leamington-562-185-Erie' as branch   from {{ source('public','erie_185') }} where drive_thru_time is not null order by date ),
erie250 as (SELECT  *,
324 as branch_id,'TH-ON-Leamington-324-250-Erie' as branch   from {{ source('public','erie_250') }} where drive_thru_time is not null order by date ),
stg_union as (
