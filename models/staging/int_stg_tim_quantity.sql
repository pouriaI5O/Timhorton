...
with stg_talbot as(SELECT  *,
        ISNULL(date, (SELECT TOP 1 date from {{ source('public','talbot_255') }} WHERE _row < t._row AND date IS NOT NULL ORDER BY _row DESC))
  from {{ source('public','talbot_255') }}  t),
  int_stg_talbot as (
  ...
  stg_maidstone as(SELECT  *,
        ISNULL(date, (SELECT TOP 1 date from {{ source('public','maidstone_1903') }} WHERE _row < t._row AND date IS NOT NULL ORDER BY _row DESC))
  from {{ source('public','maidstone_1903') }}  t),
...
stg_erie185 as(SELECT  *,
        ISNULL(date, (SELECT TOP 1 date from {{ source('public','erie_185') }} WHERE _row < t._row AND date IS NOT NULL ORDER BY _row DESC))
  from {{ source('public','erie_185') }}  t),
  int_stg_erie185 as (
  select type,sandwich_sales_quantity,coffee_sales_quantity
  ...
  stg_erie250 as(SELECT  *,
        ISNULL(date, (SELECT TOP 1 date from {{ source('public','erie_250') }} WHERE _row < t._row AND date IS NOT NULL ORDER BY _row DESC))
  from {{ source('public','erie_250') }}  t),
  int_stg_erie250 as (
  select type,sandwich_sales_quantity,coffee_sales_quantity
